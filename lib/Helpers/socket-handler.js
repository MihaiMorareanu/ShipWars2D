...
function acceptInvite(data){
	...
	.then(function(user2){
		...
		//Register new game
		return Game.update({User1: usersID[0].id, User2: usersID[1].id}, {GameRoom: roomID, BoatConfig1: [], BoatConfig2: [], startTime: '', Miss1: [], Miss2: []}, {upsert: true});
	})
...
function boatShot(data){
	...
	.then(function(game){
		...
		for(var idx=0; idx < opConfigs.length; idx++){
			...
			console.log("1 - From: " + fromHolder[0] + " - " + currCoordHolder[0] + " - " + toHolder[0]);
			console.log("2 - From: " + fromHolder[1] + " - " + currCoordHolder[1] + " - " + toHolder[1]);
			...
			if(currCoordHolder[0] >= fromHolder[0] && currCoordHolder[0] <= toHolder[0] && currCoordHolder[1] >= fromHolder[1] && currCoordHolder[1] <= toHolder[1]){
				...
				game.save(function(err, savedGame){
					if(err){
						...
						if(savedGame)
							io.sockets.emit("boat:hit", {currUser: data.currUser, Coord: hitCoord});
						else
							throw new Error("Could not load the hit!");	
					}
				});
		...
		if(isHit == false){
			...
			game.save(function(err, savedGame){
				if(err){
					...
					if(savedGame)
						io.sockets.emit("boat:miss", {currUser: data.currUser, Coord: hitCoord});
					else
						throw new Error("Could not load the hit!");	
				}
			});
