...
function getAllUsers(data){
	...
	.then(function(invites){
		...
		invites.forEach(function(invite){
			if(typeof tempInvites[invite.From.Username] == "undefined"){
				tempInvites[invite.From.Username] = [invite.To.Username];
			}else{
				tempInvites[invite.From.Username].push(invite.To.Username); 
			}
		});

		response.invites = tempInvites;
...
function startBattleRequest(data){
	...
	.then(function(user){
		...
		if(""+user._id == ""+foundGame.User1._id){
			...
			//Verify if User2 is not loggedIn
			if(foundGame.User2.isAvailable == false){
				//Remove current game
				foundGame.remove();
				
				//Increase current user points with 50
				User
				.update({'_id':foundGame.User1._id}, {Points: foundGame.User1.Points+50})
				.then(function(userUpdateRes){
					console.log("<socket-handler - startBattleRequest - opponentDisconnect> Points are increased for user: " + foundGame.User1.Username);
				}).catch(function(err){
					console.log("<socket-handler - startBattleRequest - opponentDisconnect> Error: " + err.message);
				});

				socket.emit('opponentDisconnect', {user: foundGame.User1.Username, points: foundGame.User1.Points+50});
				return;
			}
			...
			//Verify if User1 is not loggedIn
			if(foundGame.User1.isAvailable == false){
				//Remove current game
				foundGame.remove();
				
				//Increase current user points with 50
				User
				.update({'_id':foundGame.User2._id}, {Points: foundGame.User2.Points+50})
				.then(function(user){
					console.log("<socket-handler - startBattleRequest - opponentDisconnect> Points are increased for user: " + foundGame.User2.Username);
				}).catch(function(err){
					console.log("<socket-handler - startBattleRequest - opponentDisconnect> Error: " + err.message);
				});

				socket.emit('opponentDisconnect', {user: foundGame.User2.Username, points: foundGame.User2.Points+50});
				return;
			}
	...
	.then(function(game){
		...
		for(var idx=0; idx < opConfigs.length; idx++){
			...
			if(currCoordHolder[0] >= fromHolder[0] && currCoordHolder[0] <= toHolder[0] && currCoordHolder[1] >= fromHolder[1] && currCoordHolder[1] <= toHolder[1]){
				...
				if(config.Hits.indexOf(hitCoord) != -1)
					return;
				...
				game.save(function(err, savedGame){
					if(err){
						console.log("<socket-handler - gameSave> Error: " + err.message);
