...
function boatShot(data){
	...
	.then(function(game){
		...
		for(var idx=0; idx< configs.length; idx++){
			...
			if(currCoordHolder[0] >= fromHolder[0] && currCoordHolder[0] <= toHolder[0] && currCoordHolder[1] >= fromHolder[1] && currCoordHolder[1] <= toHolder[1]){
				...
				game.save(function(savedGame){
					if(savedGame)
						socket.emit("boat:hit", {currUser: data.currUser, Coord: hitCoord});
					else
		...
		if(isHit == null){
			if(isUser1)
				game.Miss1.push(data.coord);
			else
				game.Miss2.push(data.coord);

			game.save(function(savedGame){
				if(savedGame){
					socket.emit("boat:miss", {currUser: data.currUser, Coord: data.coord});
				}else{
					throw new Error('Miss could not be saved!');
				}
			});
		}
